	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if schedule != "" {
			...
			_, err := c.AddFunc(schedule, func() {
				imagescan.DiscoverAndScan(cfg, HOST_NAME, RUN_TIME, !allContainers, imagesOnly)
			})
			if err != nil {
				logger.Warn("error while schedule scanning")
				return err
			}
...
func init() {
	...
	imageScanCmd.Flags().MarkHidden("schedule")
