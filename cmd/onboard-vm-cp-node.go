...
var cpNodeCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		vmConfig, err := onboard.CreateClusterConfig(onboard.ClusterType_VM, userConfigPath, vmMode,
			...
			cidr, secureContainers, skipBTF, systemMonitorPath, rmqAddress, deploySumegine, registry, registryConfigPath, insecure, plainHTTP, preserveUpstream, topicPrefix, rmqConnectionName, sumEngineCronTime, tls, enableHostPolicyDiscovery, splunk, nodeStateRefreshTime, spireEnabled, spireCert, logRotate)
		if err != nil {
		...
		if enableVMScan {
			...
			if err != nil {
				logger.Error("error initializing RAT config", vmMode)
			}
			err = onboardConfig.InstallRAT()
			if err != nil {
				logger.Error("error installing RAT in %s mode /n", vmMode)
			}
		}
